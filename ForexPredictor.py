...
#        self.model = self.build_model()
        ...
        self.early_stopping = self.early_stopping = EarlyStopping(monitor='val_loss', patience=500)
        self.X_train = None  
        self.X_val = None  
        self.y_train = None  
        self.y_val = None  
    ...
    def train_model(self):
            ...
            self.predictions = self.model.predict(self.test_sequences)
            #self.analyze_model_with_shap()
    # placeholder methods for code not yet refactored



#    def analyze_model_with_shap(self):
#        subset_X_val = self.Y_val[:100]
#        background = self.Y_train[np.random.choice(self.X_train.shape[0], 100, replace=False)]
#        explainer = shap.KernelExplainer(self.model.predict, background)
#        shap_values = explainer.shap_values(subset_X_val)
#        shap.summary_plot(shap_values, subset_X_val)
